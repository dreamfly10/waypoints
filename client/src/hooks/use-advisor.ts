import { useMutation } from "@tanstack/react-query";
import { api } from "@shared/routes";
import { apiFetch } from "@/lib/api";

// Custom error to handle 403 easily
export class AdvisorProRequiredError extends Error {
  tokensRemaining?: number;
  constructor(message: string, tokensRemaining?: number) {
    super(message);
    this.name = "AdvisorProRequiredError";
    this.tokensRemaining = tokensRemaining;
  }
}

export function useAdvisorAsk() {
  return useMutation({
    mutationFn: async ({ query, attachedVaultItemId }: { query: string; attachedVaultItemId?: number }) => {
      const validated = api.advisor.ask.input.parse({ query, attachedVaultItemId });
      const res = await apiFetch(api.advisor.ask.path, {
        method: api.advisor.ask.method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(validated),
      });
      
      if (!res.ok) {
        if (res.status === 403) {
          const errData = api.advisor.ask.responses[403].parse(await res.json());
          throw new AdvisorProRequiredError(errData.message, errData.tokensRemaining);
        }
        throw new Error("Failed to get advisor response");
      }
      
      return api.advisor.ask.responses[200].parse(await res.json());
    },
  });
}
